# -*- coding: utf-8 -*-
# 파일명: websocket_client.py (RL 모델 통합 및 멀티 티커, 비용/슬리피지 반영)
# 실행 환경: 64비트 Python (myenv)
# 필요 라이브러리: websockets==11.0.3 tensorflow pandas numpy requests pandas_ta joblib httpx stable_baselines3
...
try:
    ...
    print("stable_baselines3 라이브러리를 찾을 수 없습니다. RL 모델 로딩이 불가능합니다. pip install stable-baselines3[extra]")
    PPO = None # PPO 사용 불가 시 None으로 설정하여 오류 방지

from gymnasium import spaces # 직접 사용은 안하지만, RL 환경 이해에 도움
...
KIWOOM_APP_KEY = "LE1PsyDirouZ8B1OxnNGrUImw-_eXshzhDwwCcweKss"
KIWOOM_APP_SECRET = "e1VMIr_CKOiBfiNgG_kKjuT7C3GQUQHnmm993AgKYNw"
KIWOOM_USER_ID = ""

IS_MOCK_TRADING = True

# --- 데이터 로드 및 저장 설정 ---
LOAD_PREVIOUSLY_SAVED_BARS = True
SAVE_LIVE_MINUTE_BARS = True

# --- 주문 로직 관련 설정 ---
ORDER_QUANTITY = 1
BUY_THRESHOLD = 0.02 # LSTM 모델용
SELL_THRESHOLD = -0.01 # LSTM 모델용
STOP_LOSS_PCT = -0.05

# --- 거래 비용 및 슬리피지 설정 ---
COMMISSION_RATE = 0.00015
TAX_RATE_SELL = 0.0020
SLIPPAGE_PCT = 0.0005

# --- 경로 설정 ---
BASE_DATA_PATH = r'G:\내 드라이브\trading_data_kiwoom'
PREFILL_DATA_PATH = os.path.join(BASE_DATA_PATH, 'prefill_minute_bars')
LIVE_BAR_SAVE_PATH = os.path.join(BASE_DATA_PATH, 'live_minute_bars')
MODEL_SAVE_PATH = r'G:\내 드라이브\lstm_models_per_stock_v2'
SCALER_SAVE_PATH = r'G:\내 드라이브\processed_stock_data_full_v2\scalers'
# RL_MODEL_SAVE_PATH = r'C:\Windows\System32' # 이전 경로, 실제 저장 경로로 수정 필요
RL_MODEL_SAVE_PATH = r'G:\내 드라이브\rl_models' # 예시: RL 모델 저장 경로 (실제 경로로 수정)
...
REGISTER_LIST = [
    {"item": ["001450", "005930", "000660"], "type": ["0B"], "fids": ["20", "10", "15"]},
]
...
class WebSocketClient:
    def __init__(self, socket_uri, token, account_no, app_key, app_secret, user_id,
                 ...
                 rl_model_dir=RL_MODEL_SAVE_PATH,
        ...
        self.rl_models = {}
        self.rl_model_path_root = rl_model_dir
        ...
        if monitored_tickers_list is None:
            self.tickers_to_monitor = []
            for reg_group in REGISTER_LIST:
                if reg_group.get("item"):
                    self.tickers_to_monitor.extend(reg_group["item"])
            self.tickers_to_monitor = sorted(list(set(self.tickers_to_monitor)))
        else:
            self.tickers_to_monitor = monitored_tickers_list

        self.positions = {}
        self.simulated_balance = 10000000 
        self.simulated_total_portfolio_value = self.simulated_balance
        ...
        for ticker in self.tickers_to_monitor:
            ...
            if self.load_previous_bars and self.prefill_data_path:
                try:
                    ...
                    if os.path.exists(csv_file_path_primary): chosen_csv_path = csv_file_path_primary
                    ...
                    if chosen_csv_path and os.path.exists(chosen_csv_path):
                        ...
                        if 'time' not in df_prefill.columns:
                            self.logger.error(f"({ticker}) CSV 파일에 'time' 컬럼 없음: {chosen_csv_path}"); continue
                        ...
                        for _, row in df_prefill.iloc[-num_bars_to_load:].iterrows():
                            try:
                                bar_data = {'time': pd.to_datetime(row['time']), 'open': float(row['open']), 'high': float(row['high']), 'low': float(row['low']), 'close': float(row['close']), 'volume': int(row['volume']), 'amount': float(row.get('amount', float(row['close']) * int(row['volume'])))}
                                self.completed_bars[ticker].append(bar_data)
                                loaded_count +=1
                            except Exception as e_row: self.logger.error(f"({ticker}) CSV 행 처리 중 오류 (건너뜀): {row}, 오류: {e_row}")
                        self.logger.info(f"({ticker}) 과거 데이터 {loaded_count}개 미리 채우기 완료: {chosen_csv_path}")
                    else: self.logger.warning(f"({ticker}) 미리 채울 과거 데이터 파일 없음: {csv_file_path_primary} 또는 {csv_file_path_live_alternative}")
                except Exception as e: self.logger.error(f"({ticker}) 과거 데이터 미리 채우기 중 오류: {e}")
            
            self.feature_sequences[ticker] = deque(maxlen=TIME_STEPS)
            if self.scaler_path_root: self.load_scalers(ticker) # load_scalers 정의 필요

            if ticker == RL_AGENT_TICKER and PPO is not None: # PPO가 임포트 되었을 때만 RL 모델 로드 시도
                self.load_rl_model(ticker)
            else:
    ...
    def load_model(self, ticker): # LSTM 모델 로더
        ...
        if os.path.exists(model_file_path):
            try:
                ...
                self.logger.info(f"LSTM 모델 로딩 성공 ({ticker}): {model_file_path}")
                return self.models[ticker]
            except Exception as e:
                self.logger.error(f"LSTM 모델 로딩 실패 ({ticker}): {e}")
                self.models[ticker] = None
                return None
        else:
            self.logger.warning(f"LSTM 모델 파일 없음 ({ticker}): {model_file_path}")
            self.models[ticker] = None
            return None

    def load_scalers(self, ticker): # 스케일러 로더 (이전에 누락되었던 부분)
        ...
        if os.path.exists(scaler_file_path):
            try:
                ...
                self.logger.info(f"스케일러 로딩 성공 ({ticker}): {len(self.scalers[ticker])}개 - {scaler_file_path}")
    ...
    def load_rl_model(self, ticker):
        if PPO is None: # stable_baselines3 임포트 실패 시
            self.logger.error("stable_baselines3가 설치되지 않아 RL 모델을 로드할 수 없습니다.")
            self.rl_models[ticker] = None
            return None
            
        if ticker in self.rl_models and self.rl_models[ticker] is not None:
            return self.rl_models[ticker]
        
        model_file_path = os.path.join(self.rl_model_path_root, f"ppo_trading_{ticker}_v2.zip")
        
        if os.path.exists(model_file_path):
            try:
                self.rl_models[ticker] = PPO.load(model_file_path)
                self.logger.info(f"RL 모델 로딩 성공 ({ticker}): {model_file_path}")
                return self.rl_models[ticker]
            except Exception as e:
                self.logger.error(f"RL 모델 로딩 실패 ({ticker}): {e}")
                self.rl_models[ticker] = None
                return None
        else:
            self.logger.warning(f"RL 모델 파일 없음 ({ticker}): {model_file_path}")
            self.rl_models[ticker] = None
            return None
    ...
    def calculate_features(self, ticker):
        ...
        valid_bars_list = [bar for bar in self.completed_bars[ticker] if all(pd.notna(bar.get(k)) for k in ['open', 'high', 'low', 'close', 'volume', 'amount'])]
        ...
        if not df_bars.empty:
            self.logger.debug(f"({ticker}) pandas_ta 입력 데이터 ('close' 컬럼 tail(5)):\n{df_bars['close'].tail(5).to_string()}")
        ...
        if not all(col in df_bars.columns for col in ['open', 'high', 'low', 'close', 'volume', 'amount']):
        ...
        try:
            ...
            # self.logger.info(f"({ticker}) df_bars columns after pandas_ta: {df_bars.columns.tolist()}") # 너무 길어서 주석 처리
            macd_cols_to_check = [f'MACD_{macd_suffix}', f'MACDs_{macd_suffix}', f'MACDh_{macd_suffix}']
            existing_macd_cols = [col for col in macd_cols_to_check if col in df_bars.columns]
            if existing_macd_cols:
                # self.logger.debug(f"({ticker}) pandas_ta 결과 MACD 관련 컬럼 tail(10):\n{df_bars[existing_macd_cols].tail(10).to_string()}") # 너무 길어서 주석 처리
        ...
        atr_col_name = f'ATRr_{self.ta_params["atr_length"]}' # pandas_ta는 ATRr_length 형태로 생성
        latest_features['ATRr_14'] = latest_bar.get(atr_col_name) # FEATURE_COLUMNS_NORM의 'ATRr_14'와 일치
        ...
        for fundamental_col in ['PBR', 'PER', 'USD_KRW']:
             if fundamental_col.lower() in df_bars.columns:
        ...
        # self.logger.debug(f"({ticker}) latest_features 값 일부: open={latest_features.get('open')}, close={latest_features.get('close')}, MACD={latest_features.get(f'MACD_{macd_suffix}')}")
        ...
        for i, col_norm_name in enumerate(self.feature_order):
            ...
            elif original_name_key_base == 'MACDs_12_26_9': current_lookup_key = f'MACDs_{macd_suffix}'
            elif original_name_key_base == 'MACDh_12_26_9': current_lookup_key = f'MACDh_{macd_suffix}'
            ...
            elif original_name_key_base == 'ATRr_14': current_lookup_key = atr_col_name
        ...
        if np.isnan(feature_vector).any():
            ...
            for idx in nan_indices:
                ...
                lookup_key_debug = original_name_key_base_debug
                ...
                elif original_name_key_base_debug == 'ATRr_14': lookup_key_debug = atr_col_name
                # ... (다른 지표들도 필요시 추가)
                nan_lookup_keys_debug.append(f"{col_norm_name_debug}(actual_lookup:'{lookup_key_debug}', value_in_latest_features: {latest_features.get(lookup_key_debug)})")
            
            self.logger.warning(f"({ticker}) 최종 피처 NaN 포함 ({len(nan_feature_names)}개). 예측 건너뜀. NaN 피처(조회키,값): {nan_lookup_keys_debug}. Vec(first 10): {feature_vector[:10]}")
            return None
        ...
        for i, col_norm_name in enumerate(self.feature_order):
            ...
            if col_norm_name.endswith('_norm'):
                ...
                if scaler:
                    ...
                    if original_name != 'is_month_end': # is_month_end는 스케일러가 없을 수 있음
                         self.logger.warning(f"({ticker}) '{original_name}' 스케일러 없음. 0.0 처리.");
                    normalized_vector[i] = raw_value if original_name == 'is_month_end' else 0.0 # is_month_end는 원본값, 나머지는 0
    ...
    def _append_bar_to_csv(self, ticker, bar_data):
        if not self.live_bar_save_path: return
        try:
        ...
        except Exception as e: self.logger.error(f"({ticker}) 1분봉 CSV 저장 오류: {e}")
    ...
    async def process_new_bar(self, ticker, bar_data):
        ...
        try:
            ...
            if len(self.feature_sequences[ticker]) == TIME_STEPS:
                ...
                action_to_take = None 
                predicted_value_for_log = None

                if ticker == RL_AGENT_TICKER and PPO is not None and ticker in self.rl_models and self.rl_models[ticker] is not None:
                    self.logger.info(f"({ticker}) RL 에이전트 기반 의사결정 시작...")
                    rl_model = self.rl_models[ticker]
                    
                    feature_sequence_array = np.array(list(self.feature_sequences[ticker])).astype(np.float32)
                    shares_held_rl = holding_info['quantity'] if holding_info else 0
                    position_status_rl = np.array([1.0 if shares_held_rl > 0 else 0.0])
                    
                    current_stock_value = shares_held_rl * current_price
                    # self.simulated_total_portfolio_value는 매매 시 업데이트되므로 여기서는 조회만
                    current_sim_portfolio_value = self.simulated_balance + current_stock_value 
                    balance_ratio_rl = np.array([self.simulated_balance / current_sim_portfolio_value if current_sim_portfolio_value > 0 else 0.0])

                    position_status_tiled_rl = np.tile(position_status_rl, (TIME_STEPS, 1))
                    balance_ratio_tiled_rl = np.tile(balance_ratio_rl, (TIME_STEPS, 1))

                    observation_2d = np.concatenate((feature_sequence_array, position_status_tiled_rl, balance_ratio_tiled_rl), axis=1).astype(np.float32)
                    observation_flat = observation_2d.flatten()

                    try:
                        rl_action, _states = rl_model.predict(observation_flat, deterministic=True)
                        action_to_take = int(rl_action)
                        predicted_value_for_log = f"RL_Action:{action_to_take}"
                        self.logger.info(f" ★★★ RL 예측 ({ticker}): Action={action_to_take} ★★★")
                    except Exception as e_rl_predict:
                        self.logger.error(f"({ticker}) RL 모델 예측 오류: {e_rl_predict}"); traceback.print_exc(); return
                else:
                    self.logger.info(f"({ticker}) LSTM 모델 기반 의사결정 시작...")
                    lstm_model = self.load_model(ticker)
                    if lstm_model:
                        ...
                        if input_seq.shape == (1, TIME_STEPS, NUM_FEATURES):
                            try:
                                ...
                                predicted_value_for_log = f"LSTM_Pred:{predicted_norm_value:.6f}"
                                self.logger.info(f" ★★★ LSTM 예측 ({ticker}): {predicted_norm_value:.6f} ★★★")
                                
                                if holding_info:
                                    if predicted_norm_value < SELL_THRESHOLD: action_to_take = 2 # SELL
                                    else: action_to_take = 0 # HOLD
                                else:
                                    if predicted_norm_value > BUY_THRESHOLD: action_to_take = 1 # BUY
                                    else: action_to_take = 0 # HOLD
                            except Exception as e_lstm_predict:
                                self.logger.error(f"({ticker}) LSTM 모델 예측 오류: {e_lstm_predict}"); traceback.print_exc(); return
                        else: self.logger.error(f"({ticker}) LSTM 모델 입력 형태 불일치: {input_seq.shape} vs (1, {TIME_STEPS}, {NUM_FEATURES})"); return
                    else: self.logger.warning(f"({ticker}) LSTM 모델 미로드. 예측 불가."); return
                
                if action_to_take is not None:
                    self.logger.info(f"({ticker}) 최종 결정된 행동: {action_to_take} (0:HOLD, 1:BUY, 2:SELL), 예측 근거: {predicted_value_for_log}")
                    self.logger.info(f"({ticker}) 현재 가격: {current_price:.0f}, 보유 정보: {holding_info}")
                    self.logger.info(f"({ticker}) 비용 설정: 수수료율={COMMISSION_RATE*100:.3f}%, 매도세율={TAX_RATE_SELL*100:.3f}%, 슬리피지={SLIPPAGE_PCT*100:.3f}%")

                    if holding_info:
                        entry_price_with_slippage = holding_info['entry_price']
                        ...
                        if current_return_with_costs <= STOP_LOSS_PCT:
                            ...
                            if order_result and order_result.get('return_code') == 0:
                                ...
                                self.simulated_balance += sell_proceeds_net
                                self.simulated_total_portfolio_value = self.simulated_balance
                                self.logger.info(f"({ticker}) 포지션 정리 (손절매 주문 성공). 실현손익(추정): {current_net_profit:.0f}, 실현수익률(추정): {current_return_with_costs*100:.2f}%. 시뮬레이션 잔고: {self.simulated_balance:.0f}")
                            else: self.logger.error(f"({ticker}) 손절매 주문 실패 또는 오류: {order_result}")
                        elif action_to_take == 2:
                            self.logger.info(f"--- 매도 신호 ({ticker}) --- 예측근거: {predicted_value_for_log}, 수익률(비용반영): {current_return_with_costs*100:.1f}%")
                            order_result = await self.send_order(order_type='sell', stock_code=ticker, quantity=quantity, trde_tp="03")
                            if order_result and order_result.get('return_code') == 0:
                                del self.positions[ticker]
                                self.simulated_balance += sell_proceeds_net
                                self.simulated_total_portfolio_value = self.simulated_balance
                                self.logger.info(f"({ticker}) 포지션 정리 (매도 신호 주문 성공). 실현손익(추정): {current_net_profit:.0f}, 실현수익률(추정): {current_return_with_costs*100:.2f}%. 시뮬레이션 잔고: {self.simulated_balance:.0f}")
                            else: self.logger.error(f"({ticker}) 매도 신호 주문 실패 또는 오류: {order_result}")
                        else: # HOLD
                            self.logger.info(f"--- 보유 유지 ({ticker}) --- 예측근거: {predicted_value_for_log}, 수익률(비용반영): {current_return_with_costs*100:.1f}%")
                    else: # 미보유 상태
                        if action_to_take == 1: # BUY
                            self.logger.info(f"+++ 매수 신호 ({ticker}) +++ 예측근거: {predicted_value_for_log}")
                            simulated_buy_price = current_price * (1 + SLIPPAGE_PCT)
                            cost_of_shares_with_commission = simulated_buy_price * ORDER_QUANTITY * (1 + COMMISSION_RATE)
                            if self.simulated_balance >= cost_of_shares_with_commission:
                                ...
                                if order_result and order_result.get('return_code') == 0:
                                    ...
                                    self.simulated_balance -= cost_of_shares_with_commission
                                    current_stock_value = ORDER_QUANTITY * current_price 
                                    self.simulated_total_portfolio_value = self.simulated_balance + current_stock_value
                                    self.logger.info(f"({ticker}) 신규 포지션 진입. 수량: {ORDER_QUANTITY}, 진입가(슬리피지반영): {simulated_buy_price:.0f}. 시뮬레이션 잔고: {self.simulated_balance:.0f}")
                                else: self.logger.error(f"({ticker}) 매수 신호 주문 실패 또는 오류: {order_result}")
                            else: self.logger.warning(f"({ticker}) 매수 신호 발생했으나 시뮬레이션 잔고 부족. (필요: {cost_of_shares_with_commission:.0f}, 현재: {self.simulated_balance:.0f})")
                        else: # HOLD
                            self.logger.info(f"--- 매수 대기 ({ticker}) --- 예측근거: {predicted_value_for_log}")
                else:
                    self.logger.info(f"({ticker}) 행동 결정 없음 (action_to_take is None). 예측근거: {predicted_value_for_log}")
    ...
    async def register_realtime(self):
        ...
        for reg_group_config in REGISTER_LIST:
            items_to_register = reg_group_config.get("item", [])
            types_to_register = reg_group_config.get("type", [])
            
            if items_to_register and types_to_register:
                if len(types_to_register) == 1 and len(items_to_register) >= 1: # type은 하나, item은 하나 이상
                    # 키움 API는 data 필드에 각 종목별 객체를 원하지 않고, 하나의 객체에 item 리스트를 원함
                    payload_object = {"item": items_to_register, "type": types_to_register}
                    kiwoom_data_list_for_request.append(payload_object)
                # 만약 각 item마다 다른 type을 지정해야 한다면, REGISTER_LIST 구조를 변경하거나 이 로직을 더 정교하게 만들어야 함
                # 현재 REGISTER_LIST 구조는 하나의 type을 여러 item에 적용하는 방식임
                else:
                    self.logger.warning(f"잘못된 등록 설정 (item/type 개수 불일치 또는 형식 오류): {reg_group_config}. 건너뜁니다.")
            else:
                 self.logger.warning(f"잘못된 등록 설정 (item 또는 type 누락): {reg_group_config}. 건너뜁니다.")

        if kiwoom_data_list_for_request:
            # 현재 REGISTER_LIST는 하나의 그룹만 있으므로 kiwoom_data_list_for_request에는 하나의 요소만 있게 됨
            # 만약 REGISTER_LIST에 여러 그룹이 있다면, 각 그룹별로 REG 요청을 보내거나, API가 여러 그룹을 하나의 요청으로 받는지 확인 필요
            # 지금은 하나의 그룹만 있다고 가정하고, 해당 그룹의 모든 종목을 하나의 REG data list로 보냄
            ...
            self.logger.warning("등록할 유효한 종목 설정이 없습니다.")
    ...
    async def send_order(self, order_type, stock_code, quantity, price=0, trde_tp="03"):
        ...
        price_str = "0" if trde_tp == "03" else str(int(price))
...
async def main():
    ...
    try:
        ...
        # RL_MODEL_SAVE_PATH는 파일이 아닌 디렉토리로 가정하고, 해당 디렉토리가 없으면 생성
        paths_to_check_or_create = [BASE_DATA_PATH, PREFILL_DATA_PATH, LIVE_BAR_SAVE_PATH, MODEL_SAVE_PATH, SCALER_SAVE_PATH]
        if RL_MODEL_SAVE_PATH and os.path.isdir(RL_MODEL_SAVE_PATH): # 경로가 디렉토리인지 확인
             paths_to_check_or_create.append(RL_MODEL_SAVE_PATH)
        elif RL_MODEL_SAVE_PATH: # 파일 경로라면 부모 디렉토리 확인
             rl_model_parent_dir = os.path.dirname(RL_MODEL_SAVE_PATH)
             if rl_model_parent_dir and not os.path.exists(rl_model_parent_dir):
                 paths_to_check_or_create.append(rl_model_parent_dir)


        for path in paths_to_check_or_create:
